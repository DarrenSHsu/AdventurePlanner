...
struct AddDestinationPhoto: View {
    // Input Parameter
    ...
    var body: some View {
        ...
        .font(.system(size: 14))
        .navigationTitle("Add New Destination Photo")
            ...
            ToolbarItem(placement: .topBarTrailing) {
                Button("Save") {
                    if inputDataValidated() {
                        ...
                        alertTitle = "New Photo Saved!"
                        alertMessage = "Your new photo is successfully saved in the database!"
                    } else {
                        showAlertMessage = true
    ...
    /*
     ****************************************
     MARK: Save New Trip Photo
     ****************************************
     */
    func saveNewTripPhoto() {
        ...
        let newPhoto = DestPhoto(photoFullFilename: photoFullFilename, dateTime: photoDateAndTimeString)
        
        // ❎ Insert it into the database context
        modelContext.insert(newPhoto)
        
        dest.destPhotos?.append(newPhoto)
